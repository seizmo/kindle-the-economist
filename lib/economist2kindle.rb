...
class EpubWriter
  def initialize(config)
    @dirs = {
        ...
        :issues => config['directories'] && config['directories']['issues'] ?
          config['directories']['issues'] : File.join("..", "issues"),
        :tmp_base => config['directories'] && config['directories']['tmp'] ?
          config['directories']['tmp'] : File.join("..", "tmp")
    }
  end
